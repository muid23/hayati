...
def home():
    """Open the Hayati interface from the main website URL."""
    return FileResponse("static/index.html")


@app.get("/chat", include_in_schema=False)
def chat_page():
    """Open the Hayati chat interface."""
    return FileResponse("static/index.html")
...
def chat(req: ChatRequest):
    ...
    if triage["level"] == "emergency":
        ...
        if assessment["results"]:
            ...
            reply += (
                f"\n\nPossible pattern to discuss with a clinician: "
                f"{top['condition']} "
                f"({top['score']}% symptom-pattern match). "
                "This is not a diagnosis and testing may be needed."
            )
    ...
    return ChatResponse(
        ...
        matched_flags=triage["matched"],
        assessment=assessment,
    )


# ---------------------------------------------------------
# ONE HEALTH SURVEILLANCE REPORT
# ---------------------------------------------------------
...
def create_report(req: ReportRequest):
    ...
    return {
        "status": "recorded",
        "message": (
            "Report recorded for surveillance analysis. "
...
async def websocket_chat(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            async for chunk in stream_gemini_async(
                history,
                message,
            ):
